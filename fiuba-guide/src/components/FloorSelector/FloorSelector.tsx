...
export const FloorSelector = ({ selectedFloor, onFloorChange }: Props) => {
    ...
    return (
        <div className={`input floor-display ${selected ? "open" : ""}`} ref={selectorRef} onClick={() => setSelected(!selected)}>
            {selected && <div className="vertical-padding-medium subtle floor-display-selector open"><FloorDisplayName floorId={selectedFloor} /></div>}
            {!selected && <div className="vertical-padding-medium emphasis floor-display-selector"> <FloorDisplayName floorId={selectedFloor} /></div>}
